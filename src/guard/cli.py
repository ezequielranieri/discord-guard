...
from guard.api.discord import DiscordClient, InvalidTokenError, DiscordAPIError
from guard.core.monitor import AccountMonitor
from guard.models.risk import RiskLevel, AccountRiskReport, Alert
...
def display_report(report: AccountRiskReport) -> None:
    ...
    if report.risks:
        for risk in report.risks:
            ...
            console.print(f"\n{emoji} [bold {color}]{risk.level.upper()}[/bold {color}] — {risk.description}")
...
async def revoke_flow(token: str, report: AccountRiskReport) -> None:
    ...
    """
    fixable_apps = [app for app in report.authorized_apps if any(
        scope in {"bot", "rpc", "messages.read", "guilds.join"} for scope in app.scopes
    )]
    ...
    if confirm:
        ...
        for app_info in fixable_apps:
            if typer.confirm(f"Revoke access for [bold red]{app_info.name}[/bold red]?"):
                try:
                    await client.revoke_app(app_info.id)
                    console.print(f"✅ [green]Successfully revoked access for {app_info.name}.[/green]")
                except DiscordAPIError as e:
                    console.print(f"❌ [red]Failed to revoke {app_info.name}: {e}[/red]")
...
@app.command()
def scan(
    token: str = typer.Option(..., "--token", "-t", help="Your Discord user token.", prompt="Enter your Discord token", hide_input=True),
) -> None:
    ...
    async def _run():
        try:
            with Progress(
                ...
                progress.add_task(description="Scanning account for report...", total=None)
                scanner = AccountScanner(token)
                report_data = await scanner.run_scan()
            ...
            if format.lower() == "pdf":
                ...
                console.print(f"\n❌ [bold red]Error:[/bold red] Unsupported format '{format}'. Use 'pdf' or 'json'.")
                return

            output_path = generator.generate(report_data, output)
            console.print(f"\n✅ [bold green]Report successfully generated:[/bold green] {output_path}")
